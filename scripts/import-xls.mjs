...
function getSku(row, idx, sheetName) {
  return String(row['sku']||row['SKU']||row['id']||`${sheetName}-${idx}`).trim();
}
...
function loadAllProducts() {
      ...
      for (const f of files) {
        ...
        try {
          ...
          for (const p of data) {
            ...
            if (!sku) continue;
            map.set(sku, {filePath: fp, category: cat, product: p});
            skuMap.set(sku, p);
          }
  ...
  console.log(`📂 Olemas kokku ${map.size} toodet ${fileMap.size} failis`);
...
function convertFile(filePath, globalMap, fileMap) {
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    if (!fileMap.has(outFile)) fileMap.set(outFile, new Map());
    ...
    for (let idx=0; idx<rows.length; idx++) {
      ...
      if (existingEntry && existingEntry.category !== newSafeCat) {
        ...
        if (oldFileMap) {
          oldFileMap.delete(sku);
          console.log(`  📦 Liigutatud SKU ${sku}: ${existingEntry.category} -> ${newSafeCat}`);
          moved++;
        }
      }
      const newProduct = {
        ...
        topCategory: newTopCat,
        subCategory: String(row['subCategory'] || 'general'),
        categoryPath: [newTopCat],
      ...
      if (!fileMap.has(targetFile)) {
        fs.mkdirSync(path.join(OUT_ROOT, newSafeCat), {recursive:true});
        fileMap.set(targetFile, new Map());
      }
      const targetMap = fileMap.get(targetFile);
      if (!globalMap.has(sku)) added++;
      else {
        const old = globalMap.get(sku).product;
        if (JSON.stringify(old) !== JSON.stringify({...old, ...newProduct})) updated++;
      }
    ...
    console.log(`  ${safeName}: +${added} uut, ♻️ ${updated} uuendatud, 📦 ${moved} liigutatud`);
  }
}
...
for (const d of dirs) if (fs.existsSync(d)) {
  try { fs.readdirSync(d).filter(f=>f.toLowerCase().endsWith('.xls')||f.toLowerCase().endsWith('.xlsx')).forEach(f=>files.push(path.join(d,f))); } catch {}
}
files=[...new Set(files)];
console.log('Leitud XLS:', files);
...
for (const [fp, skuMap] of fileMap.entries()) {
  ...
  console.log(`  ${path.relative('.',fp)}: ${arr.length} toodet`);
}
console.log('\n✅ Valmis! SKU järgi, topelt ei teki, kategooria liigutatud!');
