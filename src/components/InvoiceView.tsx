...
export const InvoiceView: React.FC<InvoiceViewProps> = ({ order, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      {/* invoice-print-root: this wrapper becomes visible during printing */}
      <div id="invoice-print-root" className="bg-white rounded-2xl max-w-4xl w-full overflow-hidden shadow-2xl relative my-6">

        {/* Action Header Bar - hidden when printing */}
        <div className="no-print bg-[#0e4da4] text-white p-4 flex flex-col sm:flex-row justify-between items-center gap-3">
          <div>
        ...
        {/* Printable Invoice - visible both on screen and in print */}
          ...
          <div className="grid grid-cols-2 gap-6 mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200">
            ...
            <div>
              ...
              {order.deliveryMethod === 'pakk' && (
                <p className="text-slate-800 font-semibold mt-1">Pakiautomaat: {order.customer.address}</p>
              )}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#0e4da4] text-white text-[11px] uppercase font-bold">
                  <th className="py-2.5 px-3 rounded-l-lg">Toode</th>
                ...
                {order.items.map(({ product, quantity }) => (
                  <tr key={product.id}>
                    <td className="py-3 px-3 font-bold text-slate-900">
                      {product.title}
                      <span className="block text-[10px] text-slate-500 font-normal">Seisukord: {product.condition}</span>
                    </td>
