...
export function CategorySidebar({ selectedL1, selectedL2, selectedL3, onSelectCategory, products }: any) {
  ...
  const grouped = useMemo(() => {
    ...
    (products || []).forEach((p: any) => {
      ...
      if (rawL1.toLowerCase().includes('desktop kasutatud')) {
        ...
        if (!gen && rawL2 && rawL2.match(/\d+-gen/i)) gen = rawL2;
      }

      const actualL2 = gen || rawL2 || 'Üldine';
      if (actualL2 === 'Üldine' &&!rawL2) return; // ära tee tühja Üldine kui pole L2
  ...
  return (
    <aside className="w-full lg:w- flex-shrink-0">
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
        ...
        <div className="space-y-1">
          <button
            ...
            className={`w-full text-left text- px-3 py-2 rounded-xl transition ${!selectedL1? 'bg-[#0e4da4] text-white font-semibold' : 'hover:bg-slate-50 text-slate-700'}`}
          >
          ...
          {Array.from(grouped.entries()).sort((a,b)=>a[0].localeCompare(b[0])).map(([base, data]) => {
            ...
            const isOpen = openGroups[base]?? isActive; // lahti kui aktiivne või käsitsi avatud
            ...
            return (
              <div key={base} className="pt-1">
                <button
                  ...
                  className={`w-full text-left text- font-semibold px-3 py-2 rounded-xl flex justify-between items-center transition ${isActive? 'bg-blue-50 text-[#0e4da4]' : 'hover:bg-slate-50 text-slate-800'}`}
                >
                  <span className="flex items-center gap-2">
                    {hasSubs && <span className={`transform transition inline-block ${isOpen? 'rotate-90' : ''}`}>›</span>}
                    {base}
                  </span>
                  <span className="text- bg-slate-100 px-2 py-0.5 rounded-full">{data.count}</span>
                </button>
                      ...
                      return (
                        <div key={sub}>
                          <button
                            ...
                            className={`w-full text-left text- px-2.5 py-1.5 rounded-lg flex justify-between transition ${isSubActive? 'bg-slate-900 text-white font-medium' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}
                          >
                            <span>{sub}</span>
                            <span className="text- opacity-60">{subData.count}</span>
                          </button>
                          {isSubActive && subData.l3.size > 0 && (
                            <div className="ml-3 mt-0.5 space-y-0.5">
                              {Array.from(subData.l3.entries()).map(([l3, cnt]) => (
                                <button key={l3} onClick={() => onSelectCategory(base, sub, l3)} className={`w-full text-left text- px-2 py-1 rounded ${safeLower(selectedL3)===safeLower(l3)?'bg-blue-50 text-[#0e4da4] font-bold':'text-slate-500 hover:bg-slate-50'}`}>{l3} ({cnt})</button>
                              ))}
                            </div>
                          )}
                        </div>
